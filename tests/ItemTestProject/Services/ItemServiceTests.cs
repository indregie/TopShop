...
public class ItemServiceTests
{
    ...
    public async Task Add_GivenAddDto_ReturnsResponseDto()
    {
        ...
        //Assert 
        _itemRepositoryMock.Verify(x => x.Add(It.IsAny<Item>()), Times.Once);
    ...
    public async Task Delete_GivenInvalidId_ThrowsItemNotFoundException()
    {
        //Arrange
        var guid = Guid.NewGuid();
        _itemRepositoryMock.Setup(m => m.Get(guid)).Returns(Task.FromResult<Item>(null));

        //Act Assert
        await Assert.ThrowsAsync<ItemNotFoundException>(() => _itemService.Delete(guid));      
    }
